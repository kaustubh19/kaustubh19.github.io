"use strict";
/**
 * application model repository
 */
var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var PagesSchema = require("./../dataAccess/schemas/PagesSchema");
var RepositoryBase = require("./BaseRepository");
var PagesRepository = (function (_super) {
    __extends(PagesRepository, _super);
    function PagesRepository() {
        return _super.call(this, PagesSchema) || this;
    }
    return PagesRepository;
}(RepositoryBase));
Object.seal(PagesRepository);
module.exports = PagesRepository;
//# sourceMappingURL=PagesRepository.js.map