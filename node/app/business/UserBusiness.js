...
var UserBusiness = (function () {
    ...
    UserBusiness.prototype.retrieve = function (query, callback) {
        this._userRepository.retrieve(query, callback);
    };
    UserBusiness.prototype.aggregate = function (query, callback) {
        this._userRepository.aggregate(query, callback);
    };
    UserBusiness.prototype.retrieveFields = function (query, fields, callback) {
    ...
    UserBusiness.prototype.deleteMany = function (query, callback) {
    ...
    UserBusiness.prototype.count = function (query, callback) {
    ...
    UserBusiness.prototype.createToken = function (user) {
        //expiresIn seconds
        return jwt.sign({ email: user.email }, '93c7ec1855c74462f00a62degecaef8d', { expiresIn: 60 * 60 * 5 });
    };
    UserBusiness.prototype.verifyToken = function (req, res, callback) {
    ...
    UserBusiness.prototype.generateUserPdf = function (user, imageData) {
        console.log("inpdf");
        try {
            var agentdata = [];
            ...
            if (user[0].education && user[0].education.length > 0) {
                user[0].education.forEach(function (educations, index) {
            ...
            if (user[0].skills && user[0].skills.length > 0) {
                user[0].skills.forEach(function (skill, index) {
                    if (index >= 3) {
                        skilldata.push("<span>\n                      " + skill.keywordval + "\n                   ,</span>");
                    }
                });
            ...
            if (user[0].professionalSummary && user[0].professionalSummary.length > 0) {
                user[0].professionalSummary.forEach(function (professionalsum, index) {
